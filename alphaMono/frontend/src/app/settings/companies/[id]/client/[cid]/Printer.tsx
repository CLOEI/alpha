...
import { HardDriveDownload, Plus, PrinterIcon } from "lucide-react";
...
function Credentials({ data, refetch, id }: any) {
  const [addMapping] = useMutation(gql`
    mutation AddPrinter($specClientId: Int!, $name: String!) {
      addPrinter(SpecClientID: $specClientId, name: $name) {
        name
      }
    }
  `);
